...
void Node::Deactivate(Node *node) {
  ROS_INFO("Node::Deactivate was called!!!!\n");
  // undo current node
  if(node->state_.active || node->state_.done) {
    node->state_.active = false;
    node->state_.done = false;
    node->state_.activation_level = 0.0f;
    node->state_.activation_potential = 0.0f;
  }

 // send undo message to peers
  robotics_task_tree_msgs::ControlMessage msg;
  msg.peerUndone = false;
  DeactivatePeer(node->mask_, msg);
...
void Node::DeactivateNode(Node *node) {
      ...
      // pause architecture
      pause_thread = new boost::thread(&Deactivate,node);
      // wait until node is deactivated to continue
      pause_thread->join();
      return;
}

void Node::DeactivatePeer(NodeBitmask node, const robotics_task_tree_msgs::ControlMessage msg) {
