...
Node::Node(NodeId_t name, NodeList peers, NodeList children, NodeId_t parent,
  ...
  // undo function
  state_.tempDeactivate = false;
  ros::Subscriber undoSub = sub_nh_.subscribe("undoSub", 1000, &Node::undoCallback,this);
}
...
void Node::undoCallback(ConstControlMessagePtr_t msg) {
  ...
  state_.tempDeactivate = true;
  DeactivateNode();
}

void Node::dropCallback(ConstControlMessagePtr_t msg) {
  ROS_INFO("[%s]: Node::dropCallback was called!!!!\n\n\n\n\n", name_->topic.c_str());
  boost::unique_lock<boost::mutex> lck(mut);
...
void Node::DialogueCallback(const dialogue::Resolution::ConstPtr &msg, bool& done) {
  done = true;
...
void Node::Dialogue() {
  ...
  ros::NodeHandle n;
  ros::Publisher init_dialogue_pub = n.advertise<dialogue::Issue>("issues", 1000);
  ...
  init_dialogue_pub.publish(msg);
  ros::spinOnce();
  // loop_rate.sleep();
  ROS_INFO("GOT TO DIALOGUE PUB!!!!!!!\n\n\n\n\n\n\n\n\n\n");

  bool resp_received = false;
  ros::Subscriber dialogue_resp_sub = n.subscribe<dialogue::Resolution>("resolution", 1000, boost::bind(&Node::DialogueCallback, this, _1, resp_received));
  // wait for response
  while(!resp_received) {
    ros::spinOnce();
...
void Node::PublishStateToChildren() {
  ...
  msg->active = state_.active;
  msg->parent_type = state_.parent_type;
  //msg->peerUndone = state_.tempDeactivate;
